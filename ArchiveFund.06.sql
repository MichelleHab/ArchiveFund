...
INSERT INTO `User` VALUES (1,'Скрыльников Дмитрий Константинович','Admin','derector_sdk','b66dd5a7a689f88e302ab2ae4a9567f9c7572c18e520b3bf712bb2630b3931a503d647baedf48df470006312d07984216578b60526e5ee6137ef1fd215190a0c'),(2,'Федусева Элла Юрьевна','Employer','fedusia_eu','756ff174383797900c043b75fee29df16ee168174c59286cd3cea59418d4d5619035b13202262c4b3726c9eee93d679187f8286420b6fb4774be1d3fb720cb32'),(3,'Нуралиев Арсен Абдулжалилович','Employer','nuraliev_aa','97b2dfe1f6cc284e35c0a273871c34422c34759058bc498ac2ba231ee317ca81155260920d8a8f9a11593ad6215f0afb288a1354057a2a3ef9b15443be06fdda'),(4,'Мартыненко Вадим Алексеевич','Admin','martinenko_cool','d9228615ddb7d87b33eb9e7957a706a5ef4db3a1bffe9962f364a0597966a42ac3b76ff5cb94212f9c15ae081412352e6167853b3554e7449391660c4e32824c'),(5,'Скрыльникова Наталья Владимировна','Employer','scrilnikova_nv','ab53805cfc59a5294b8f6a4739d3c2778207b01aeab4b3d9c05e1d3d2d5fe02fd77f148c7eebc637582ff70471e66a0ffce9ace5c8b1a14735eb8248d2414323'),(6,'Истомина Анна Николаевна','Admin','upravlauszh_an','78dc20c6e141a3688dd406c1e6f906a6b8256417db4059b41d09e16776b7aaa6913432d68215bc0ae88302564f24cfa7d1c670b8968af4fe4a033d5901c6d809'),(7,'Попова Вероника Владимировна','Employer','popova_tak_nado','6ad9da39ad65249214704ec4721e7478229919bdb0dd3daf0209424b700c5ee365285efde2f161b1eac73b8cca15b15a4affea9488e9eb77a0a46106fbe4e69c'),(8,'Павлов Андрей','Admin','very_developer','ca8009b0cd89a3010fc259b08cab1a20cc684cd44ad1574adb9edbefd1d86cdf7471b3a2e8085dd23dc39a0f6c0aac2228b0fb4ced4e227d354b3ea5320de984'),(9,'Лебедев Олег','Admin','developer_lo','63209b326acadd6d0bce5d83be2cf810479c843469082a8899638d0478ef0d5dc73a0822258dccd0bf7532c9f25951baec2310d9ea1bcd34e7676d4d56861024');
/*!40000 ALTER TABLE `User` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Dumping routines for database 'ArchiveFund'
--
/*!50003 DROP FUNCTION IF EXISTS `GetBoxCountByType` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `GetBoxCountByType`(p_type_id INT) RETURNS int
    READS SQL DATA
    DETERMINISTIC
BEGIN
    DECLARE box_count INT DEFAULT 0;
...
/*!50003 DROP FUNCTION IF EXISTS `HasActiveDocuments` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` FUNCTION `HasActiveDocuments`(p_student_id INT) RETURNS tinyint(1)
    READS SQL DATA
    DETERMINISTIC
BEGIN
    DECLARE has_docs BOOLEAN DEFAULT FALSE;

    -- Проверяем, есть ли у студента активные документы
    SELECT COUNT(*) > 0 INTO has_docs
    FROM Documents
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `ArchiveStudent` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `ArchiveStudent`(
    IN p_student_id INT,
    IN p_deduction_reason VARCHAR(255),
...
BEGIN
    ...
    BEGIN
        ...
        ROLLBACK;
        SET p_result_message = 'Ошибка: Транзакция откачена. Не удалось архивировать студента.';
    END;

    -- Начало транзакции
    START TRANSACTION;

    -- Проверка существования студента через функцию HasActiveDocuments
    IF HasActiveDocuments(p_student_id) THEN
        -- 1. Копируем данные студента в таблицу удалённых личных дел
        INSERT INTO DeletedStudentsPersFiles (deduction_year, reason, admission_year, student_id)
        SELECT
            CURDATE() AS deduction_year,
            p_deduction_reason AS reason,
            admission_year,
            student_id
        FROM StudentsPersFiles
        WHERE student_id = p_student_id;

        -- 2. Копируем документы студента в таблицу удалённых документов
        INSERT INTO DeletedDocuments (document_subject, start_data, type_id, Supervisor_full_name, student_id, box_id)
        SELECT
            document_subject,
            start_data,
            type_id,
        ...
        -- 3. Получаем количество коробок определённого типа для отчёта
        SELECT GetBoxCountByType(1) INTO @box_count_report;

        -- 4. Удаляем документы студента из активной таблицы
        DELETE FROM Documents WHERE student_id = p_student_id;

        -- 5. Удаляем личное дело студента из активной таблицы
        DELETE FROM StudentsPersFiles WHERE student_id = p_student_id;

        -- 6. Удаляем студента (каскадно удалит связанные записи)
        DELETE FROM Student WHERE student_id = p_student_id;

        -- Фиксация транзакции
        COMMIT;

        SET p_result_message = CONCAT('Успешно: Студент архивирован. Отчёт по коробкам типа 1: ', @box_count_report);
    ELSE
        -- Если нет активных документов, откатываем транзакцию
        ROLLBACK;
        SET p_result_message = 'Ошибка: У студента нет активных документов для архивации.';
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2026-04-24 17:10:33
